...
class MyApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      home: const OTP_afterSignUp(),
    );
  }
}

class OTP_afterSignUp extends StatefulWidget {
  final String? email;
  final String? password;
  final String? firstName;
  final String? lastName;

  const OTP_afterSignUp({
    Key? key,
    this.email,
    this.password,
    this.firstName,
  ...
  @override
  State<OTP_afterSignUp> createState() => _OTP_afterSignUpState();
}

class _OTP_afterSignUpState extends State<OTP_afterSignUp> {
    ...
    try {
      ...
      if (userCredential.user != null) {
        ...
        String phoneNumberWithoutCode =
            rawPhoneNumber ?? ""; // Default to empty string if null

        // Check for each country code and remove it if present
        if (rawPhoneNumber != null) {
          if (rawPhoneNumber.startsWith('+966')) {
            // Remove '+966' for Saudi Arabia
            phoneNumberWithoutCode = rawPhoneNumber.substring(4);
          } else if (rawPhoneNumber.startsWith('+971')) {
            // Remove '+971' for United Arab Emirates
            phoneNumberWithoutCode = rawPhoneNumber.substring(4);
          } else if (rawPhoneNumber.startsWith('+1')) {
            // Remove '+1' for United States
            // This assumes the phone number is in the format +1XXX... where XXX... is the local number.
            phoneNumberWithoutCode = rawPhoneNumber.substring(2);
          }
        }

        print('Raw phone number: $rawPhoneNumber');
        print('Phone number without code: $phoneNumberWithoutCode');
