...
import 'package:maazim/AccountDeletionService.dart';
...
Future<void> deleteEventsForUser(String userID) async {
  try {
    ...
    // Send notifications to invitees _sendNotificationsToInvitees();
  } catch (error) {
    print('Error deleting events: $error');
  }
}

Future<void> _sendNotificationsToInvitees() async {
  try {
    ...
    for (QueryDocumentSnapshot eventDoc in eventsSnapshot.docs) {
      ...
      for (QueryDocumentSnapshot inviteeDoc in inviteesSnapshot.docs) {
        ...
        await _sendNotification(inviteeID, eventName);
...
Future<void> _sendNotification(String userID, String eventName) async {
  try {
    ...
    // For example, if you're using a notification package like 'awesome_notifications':
